...
const PeriodCard: React.FC<PeriodCardProps> = ({
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get('http://localhost:3000/student/class', {
...
// console.log(data.subjects.map((student: any) => student.name))
  ...
  const handleAttendance = async (userId: string,periodId:string,status:string) => {
    const response = await axios.put('http://localhost:3000/student/attendance', { userId,periodId,status });
