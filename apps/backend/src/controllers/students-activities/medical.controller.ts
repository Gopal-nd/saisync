...
export const getAllMedical= asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user?.userId;

  if (!userId) throw new APIError({ message: "Unauthorized", status: 401 });

  const projects = await prisma.medicalLeaves.findMany({
    where: {
...
export const getMedicalById = asyncHandler(async (req: Request, res: Response) => {
  const project = await prisma.medicalLeaves.findUnique({
...
export const createMedical = asyncHandler(async (req: Request, res: Response) => {
    ...
    const project = await prisma.medicalLeaves.create({
      data: {
        title,
        description,
        userId,
