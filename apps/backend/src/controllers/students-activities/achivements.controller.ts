...
export const getAllAchivements = asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user?.userId;

  if (!userId) throw new APIError({ message: "Unauthorized", status: 401 });

  const participations = await prisma.achievements.findMany({
    where: {
...
export const getAchivementById = asyncHandler(async (req: Request, res: Response) => {
  const project = await prisma.achievements.findUnique({
...
export const createAchivement = asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user?.userId;
...
export const updateAchivements = asyncHandler(async (req: Request, res: Response) => {
    const {
        title,
        description,
        startDate,
...
export const deleteAchivements = asyncHandler(async (req: Request, res: Response) => {
